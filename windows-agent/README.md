...
- Las llamadas P/Invoke corresponden a la API C Liblinphone 5.5: `linphone_factory_get`, `linphone_factory_set_top_resources_dir`, `linphone_factory_set_msplugins_dir`, `linphone_factory_create_core_3`, `linphone_core_enable_database`, `linphone_core_start`, `linphone_core_get_extended_audio_devices`, `linphone_core_iterate`, `linphone_core_stop` y `linphone_core_unref`. Para enumerar la lista y liberar sus objetos se usan las funciones públicas de lista de Bctoolbox.
...
- La sonda localiza el plugin WASAPI `libmswasapi.dll` incluido en el paquete y configura su carpeta con `linphone_factory_set_msplugins_dir` antes de crear el Core. Si no lo encuentra, termina con un error indicando la ruta esperada.
- El Core se crea con rutas de configuración nulas, por lo que la sonda no carga un archivo de cuentas. Antes de iniciarlo llama a `linphone_core_enable_database(core, false)`: no necesita la base interna de Linphone para enumerar audio y así evita acceder a `%LOCALAPPDATA%\linphone\linphone.db`. `linphone_core_iterate()` se ejecuta cada 20 ms en el hilo principal, según la documentación.
- Si `linphone_core_start` falla, la sonda muestra el código de retorno. No libera el Core en ese caso: el SDK 5.5 puede dejarlo en estado Startup y `unref` dispara una transición nativa inválida; el proceso termina inmediatamente para que Windows recupere sus recursos.
...
Get-ChildItem .\windows-agent\src\AgenDial.LinphoneProbe\bin\x64\Release\net9.0 -Recurse -File | Where-Object { $_.Name -eq 'vcard_grammar.belr' -or $_.Name -eq 'liblinphone.dll' -or $_.Name -eq 'bctoolbox.dll' -or $_.Name -eq 'libmswasapi.dll' } | Select-Object -ExpandProperty FullName
```

Si el build tiene éxito pero la ejecución falla, adjunta el mensaje completo y el listado de DLL, en particular la ruta impresa después de `Plugins multimedia SDK:`. Si la sonda inicia, deja visibles `Core iniciado.` y los dispositivos detectados; luego detén con Ctrl+C y confirma `Core detenido correctamente.`. Indica la edición y versión de Windows (`winver`) y la arquitectura del equipo. Revisa los logs antes de compartirlos.
